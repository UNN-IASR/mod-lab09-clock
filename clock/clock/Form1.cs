...
namespace clockApplication
{
    ...
    {
        int clockRadius = 300, secondLength = 270, minuteLength = 230, hourLength = 190;
        public Form1()
        {
            InitializeComponent();
            Paint += Draw;
        ...
        private void Draw(object sender, PaintEventArgs e)
        {
            ...
            graph.DrawEllipse(new Pen(new SolidBrush(Color.DarkBlue), 4), clockRectangle);
            ...
            {
                graph.DrawLine(
                    new Pen(new SolidBrush(Color.DarkBlue), 4),
            ...
            graph.DrawLine(new Pen(new SolidBrush(Color.Cyan), 4), center, Point.Add(center, new Size(0, -secondLength)));
            graph.Restore(save);
            save = graph.Save();

            graph.RotateTransform(minuteAngle);
            graph.DrawLine(new Pen(new SolidBrush(Color.LightSkyBlue), 4), center, Point.Add(center, new Size(0, -minuteLength)));
            graph.Restore(save);

            save = graph.Save();
            graph.RotateTransform(hourAngle);
            graph.DrawLine(new Pen(new SolidBrush(Color.Magenta), 4), center, Point.Add(center, new Size(0, -hourLength)));
            graph.Restore(save);
