...
namespace lab9
{
    public partial class Form1 : Form
    {
        ...
        private void Draw(object sender, PaintEventArgs e)
        {
            ...
            graph.DrawLine(new Pen(new SolidBrush(Color.Green), 4), center, Point.Add(center, new Size(0, -secondLength)));
            graph.Restore(save);
            save = graph.Save();

            graph.RotateTransform(minuteAngle);
            graph.DrawLine(new Pen(new SolidBrush(Color.Red), 4), center, Point.Add(center, new Size(0, -minuteLength)));
            graph.Restore(save);

            save = graph.Save();
            graph.RotateTransform(hourAngle);
            graph.DrawLine(new Pen(new SolidBrush(Color.DarkBlue), 4), center, Point.Add(center, new Size(0, -hourLength)));
            graph.Restore(save);
